function PokemonCard({ pokemon, typeColors, onClick }) {
    ...
    return (
      <div
        onClick={onClick}
        className={`p-4 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 hover:scale-105 cursor-pointer ${gradientColor} border-2 border-opacity-50 border-${primaryType.toLowerCase()}-500 dark:border-opacity-70`}
      >
        <img
          src={pokemon.image}
          alt={pokemon.name}
          className="w-28 h-28 mx-auto mb-2 transform hover:rotate-6 transition-transform duration-200"
        />
